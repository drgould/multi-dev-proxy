...
```sh
# Managed by mdp — mdp-managed keys are updated on start.
API_PORT="42301"
...
Values written by `mdp` are double-quoted so the file is safe to `source` from a shell or load with standard `.env` parsers. When the file is created from scratch, keys are sorted alphabetically.

If the file already exists, `mdp` updates only the lines for keys it manages (in place, preserving an `export ` prefix and any inline comment) and appends new keys at the end in sorted order — comments, blank lines, unrelated variables, and their order are left untouched. A consequence: a key removed from `mdp.yaml` lingers in the file with its last-written value until you delete the line by hand.
...
- **`mdp run -i`**: each input is prompted in declaration order through an interactive terminal UI, which requires both stdin and stderr to be a real terminal (it errors if either is piped or redirected). A `choices: groups` input shows the active groups (filtered by `repo` if set) plus a final `@{current}` entry — "this checkout's default group" — for [links](#links) that should fall back to the caller's own group (marking the `default`); use Up/Down to browse the list (each selection fills the text box) or type a custom value, and press Enter to submit whatever's in the box. A `choices: groups` input with no matching active groups and a declared `default` is skipped silently — exactly like plain `mdp run` — so `-i` only prompts when there is actually something to select; with no `default` it degrades to a free-text prompt since a value is still required. If the group listing fails (e.g. the orchestrator can't be reached), the input also falls back to a free-text prompt rather than silently taking the `default`. Submitting an input that has no `default` with an empty box re-prompts; `Esc` or `Ctrl-C` aborts the run.
...
When `mdp run` is connected to a terminal (unix only), hooks run on a pseudo-terminal so a hook that prompts for input doesn't hang silently. If a hook's output stalls mid-line (or right after a line ending in `:`, `?`, or `>`), the hook takes focus of your terminal: the prompt is replayed, keystrokes go to the hook (`Ctrl-C` included), and all other service/log output is buffered in memory until focus releases — then it flushes and streaming resumes. Focus releases automatically once the hook moves past its prompt (resumes output that no longer looks like a prompt), when the hook exits, or when you press `Ctrl-]`. Before you type anything, an unterminated prompt holds focus even if the hook prints noise (a countdown, a periodic log line) while waiting. If several hooks need input, they take focus one at a time, with the in-between output flushed between them. Because a PTY has a single output stream, hook stderr is merged into stdout in this mode. This applies to `setup`, `shutdown`, and [`post_start`](#post_start-hooks) hooks. Set `MDP_NO_HOOK_PTY=1` to disable and run hooks on plain pipes; non-TTY environments (CI) and Windows always use plain pipes.
...
The mdp-managed keys mirror exactly what the `api` process sees (merged into the existing file if one is already there, same rules as `global.env_file` — any other keys in the file are preserved but not passed to the process):
